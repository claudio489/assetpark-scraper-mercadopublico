...
exports.scoreOpportunities = scoreOpportunities;
exports.scoreSingle = scoreSingle;
 ...
 * < 60  → baja
 */
function scoreOpportunities(opportunities, profile) {
    return opportunities.map(opp => scoreSingle(opp, profile));
}
function scoreSingle(opportunity, profile) {
...
function calculateMatchScore(opp, profile) {
    ...
    if (profile.regions.length === 0 || profile.regions.includes('Todas')) {
        ...
        const regionMatch = profile.regions.some(r => opp.region.toLowerCase().includes(r.toLowerCase()));
        if (regionMatch)
            score += 20;
    }
 ...
 */
function simulateAIScore(opp, profile) {
...
function findMatchedKeywords(opp, profile) {
    const text = `${opp.title} ${opp.description} ${opp.category}`.toLowerCase();
