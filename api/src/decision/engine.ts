// ==========================================
// DECISION ENGINE v5.1 - Core Engine
// Scoring avanzado: P, Fit, V, R, S
// Sin dependencias externas (pure JS/TS)
// ==========================================
...
function calcProbabilitySuccess(op: Opportunity): number {
  ...
  let recBonus = 0;
  if (op.recommendationV50 === 'POSTULAR') recBonus = 0.5;
  else if (op.recommendationV50 === 'EVALUAR') recBonus = 0.1;
  else recBonus = -0.3;
...
function calcFit(op: Opportunity, ex: Executor): number {
  // A) Match de industria
  const industryMatch = ex.industries.includes(op.industry) ? 1.0 : 0.0;
  if (industryMatch === 0) return 0; // no fit si no esta en la industria

  // B) Rango de monto
  let amountFit = 1.0;
  if (op.value > 0) {
    if (op.value < ex.minAmount) amountFit = 0.2;
    else if (op.value > ex.maxAmount) amountFit = 0.1;
    else {
...
function calcExpectedProfit(op: Opportunity, ex: Executor, prob: number, fit: number): number {
...
function estimateValueByIndustry(industry: string): number {
...
function calcRisk(op: Opportunity, ex: Executor): number {
  // w1: financial — monto desconocido o fuera de rango
  let financialRisk = 0;
  if (op.value === 0) financialRisk = 70; // monto oculto = alto riesgo
  else if (op.value < ex.minAmount) financialRisk = 50;
  else if (op.value > ex.maxAmount) financialRisk = 60;
  else financialRisk = 20;
...
function calcDecisionValue(expectedProfit: number, fit: number, risk: number, op: Opportunity): number {
  ...
  if (op.recommendationV50 === 'POSTULAR') s += 10;
  // Penalizacion para EVITAR v5.0
  if (op.recommendationV50 === 'EVITAR') s -= 15;

  return Math.max(0, Math.min(100, Math.round(s)));
}

// Decision binaria basada en score
function calcDecision(score: number, prob: number, risk: number, ex: Executor): 'ACCEPT' | 'REJECT' | 'MONITOR' | 'PARTNER' {
...
export function analyzeOpportunity(op: Opportunity, executors: Executor[]): DecisionScores {
  // 1. Probabilidad de exito
  const prob = calcProbabilitySuccess(op);

  // 2 & 3 & 4: Evaluar contra cada executor y elegir el mejor
  let bestExecutor: Executor | null = null;
  let bestFit = -1;
  let bestProfit = 0;
  let bestRisk = 100;
